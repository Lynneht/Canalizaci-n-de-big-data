...
import requests # Consultar a paginas web o apis
...
externar_data = requests.get(URL).text # string
...
for i in range(0, 20, 1):
  ...
  gender = external_json['rsults'][i]['gender']
  origin = external_json['results'][i]['origen']['name']
  location = external_json['resdults'][i]['location']['name']
